...
def build_vision_data(hf_split, pipe, label_names, dataset_name, batch_size=8):
    dataset = BeansDataset(load_dataset(dataset_name, split=hf_split))
...
def run_quality_checks(pipe, label_names, dataset_name):
    ...
    train_data = build_vision_data("train", pipe, label_names, dataset_name)

    print("Building test VisionData...")
    test_data = build_vision_data("test", pipe, label_names, dataset_name)
